...
-- drop table tb_orden_pedido_detalle
...
begin
	select u.iduser,u.nombre,u.apellido,u.username,u.clave,u.email,u.fecnac,u.idpais,
	sum(dp.cantidad) as 'Suma'
	 from tb_orden_pedido_detalle as dp join orden_pedido cp
		on dp.idPedido = cp.idPedido join usuario as u
...
/*Procedimiento para el listado del skins*/
select i.iditem,i.nombreItem,i.precio,t.destipo,r.descripcion 
from  
item i join tipo t on i.idtipo = t.idtipo 
join rarity r on i.idrarity= r.idrarity where t.idtipo = 1;
...
select * from tb_orden_item_detalle;
...
declare 
	v_pavos int;
begin
	set v_pavos = (select pavos from usuario as u join orden_item oi
	on oi.idusuario = u.iduser where oi.idcompra=p_idcompra);
	if
	  v_pavos < p_precio then
		 'No Corresponde al precio'
	else
